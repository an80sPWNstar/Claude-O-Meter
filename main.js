...
function showHelp(parent) {
  const detail = [
    ...
    '',
    'If the chip says RE-LOGIN NEEDED, cswap\'s stored login for that account',
    'was rejected. Fix it in a terminal:',
    '    claude          then /login as that account',
    '    cswap add       stores the fresh login',
    'This happens when two things keep separate copies of one account\'s login —',
    'for example a second CLAUDE_CONFIG_DIR profile alongside cswap. Logging in',
    'rotates the token and silently invalidates the other copy. Running Claude',
    'Code through "cswap run <number>" keeps a single copy and avoids it.',
    '',
